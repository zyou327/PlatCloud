...
jQuery.fn.timelinr = function (options) {
    ...
    $(function () {
        ...
        if (settings.orientation == 'horizontal') {
            $(settings.issuesDiv).width($(settings.issuesDiv).parent().width());
            $(settings.issuesDiv).find('img').width($(settings.issuesDiv).parent().width());
            ...
            if (settings.orientation == 'horizontal') {
                if (!$(settings.issuesDiv + ' li:last-child').hasClass(settings.issuesSelectedClass)){
                    ...
                    if (currentPositionIssues <= -(widthIssue * howManyIssues - (widthIssue))) {
                        ...
                        $(settings.datesDiv + ' a.' + settings.datesSelectedClass).parent().next().children().click();
                    } else {
                ...
                if (currentPositionIssues <= -(heightIssue * howManyIssues - (heightIssue))) {
                    ...
                    if (!$(settings.issuesDiv).is(':animated')) {
                        $(settings.issuesDiv).animate({'marginTop': currentPositionIssues - heightIssue}, {
                            queue: false,
                            duration: settings.issuesSpeed
                        });
                        $(settings.issuesDiv + ' li').animate({'opacity': settings.issuesTransparency}, {
            ...
            if (settings.orientation == 'horizontal') {
                if (!$(settings.issuesDiv + ' li:first-child').hasClass(settings.issuesSelectedClass)){
                    ...
                    if (currentPositionIssues >= 0) {
                        ...
                        $(settings.datesDiv + ' a.' + settings.datesSelectedClass).parent().prev().children().click();
                    } else {
                        if (!$(settings.issuesDiv).is(':animated')) {
                            //$(settings.issuesDiv).animate({'marginLeft':currentPositionIssues+widthIssue},{queue:false, duration:settings.issuesSpeed});
                            $(settings.issuesDiv + ' li').animate({'opacity': settings.issuesTransparency}, {
